...
public class ShadowLayout extends FrameLayout {
    ...
    public void getAttrs(@NonNull Context context, AttributeSet attrs) {
        int defaultElevation = (int) context.getResources().getDimension(R.dimen.shadow_view_elevation);
        int defaultCornerRadius = (int) context.getResources().getDimension(R.dimen.shadow_view_corner_radius);

        if (attrs != null) {
            //get attrs array
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ShadowLayout);
            //get all attributes
            SHAPE_TYPE = a.getString(R.styleable.ShadowLayout_shadow_view_type);
            if (SHAPE_TYPE == null) {
                SHAPE_TYPE = SHAPE_TYPE_RECTANGLE;
            }

            SHADOW_TYPE = a.getString(R.styleable.ShadowLayout_shadow_shadow_type);
            if (SHADOW_TYPE == null) {
                SHADOW_TYPE = SHADOW_TYPE_OUTER;
            }

            ELEVATION = a.getDimensionPixelSize(R.styleable.ShadowLayout_shadow_elevation, defaultElevation);
            CORNER_RADIUS = a.getDimensionPixelSize(R.styleable.ShadowLayout_shadow_corner_radius, defaultCornerRadius);
            BACKGROUND_COLOR = a.getColor(R.styleable.ShadowLayout_shadow_background_color, ContextCompat.getColor(context, R.color.shadow_background_color));
            SHADOW_COLOR = a.getColor(R.styleable.ShadowLayout_shadow_shadow_color, ContextCompat.getColor(context, R.color.shadow_shadow_color));
            HIGHLIGHT_COLOR = a.getColor(R.styleable.ShadowLayout_shadow_highlight_color, ContextCompat.getColor(context, R.color.shadow_highlight_color));
            SHADOW_VISIBLE = a.getBoolean(R.styleable.ShadowLayout_shadow_shadow_visible, true);
            String layerType = a.getString(R.styleable.ShadowLayout_shadow_layer_type);
            ...
            BACKGROUND_COLOR = ContextCompat.getColor(context, R.color.shadow_background_color);
            SHADOW_COLOR = ContextCompat.getColor(context, R.color.shadow_shadow_color);
            HIGHLIGHT_COLOR = ContextCompat.getColor(context, R.color.shadow_highlight_color);
    ...
    private void resetPath(int w, int h) {
        ...
        switch (SHAPE_TYPE) {
            case SHAPE_TYPE_CIRCLE:
                ...
                int maxDiameter = Math.min(this.getWidth(), this.getHeight());
